...
const GAME_ID = 'minzzle-swipes';

const MinzzleSwipesLevelsPage = () => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl w-full">
            {levels.map((level, i) => (
              <button
                ...
                className="group relative rounded-xl bg-card border border-border/50 p-6 text-left transition-all duration-300 hover:scale-[1.03] hover:border-neon-orange/40 animate-fade-in-up"
              ...
              >
                    ...
                    {Array.from({ length: 5 }).map((_, si) => (
                      <div
                        ...
                        className={`w-1.5 h-1.5 rounded-full ${
                          si < level.difficulty ? 'bg-neon-orange' : 'bg-muted'
                        }`}
                      />
                    ))}
                  </div>
                ...
                <div className="absolute inset-0 rounded-xl bg-neon-orange/5 opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
            ))}
          </div>
        )}
...
export default MinzzleSwipesLevelsPage;
