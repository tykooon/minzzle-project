...
const GAME_ID = 'minzzle-swipes-hex';

const MinzzleSwipesHexLevelsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b border-border/50 px-6 py-5 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-display font-bold neon-text tracking-wider">SWIPES HEX</h1>
        ...
        {levels && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl w-full">
            {levels.map((level, i) => (
              <button
                key={level.id}
                onClick={() => navigate(`/${GAME_ID}/play/${level.id}`)}
                className="group relative rounded-xl bg-card border border-border/50 p-6 text-left transition-all duration-300 hover:scale-[1.03] hover:border-neon-purple/40 animate-fade-in-up"
              ...
              >
                    ...
                    {Array.from({ length: 5 }).map((_, si) => (
                      <div
                        ...
                        className={`w-1.5 h-1.5 rounded-full ${
                          si < level.difficulty ? 'bg-neon-purple' : 'bg-muted'
                        }`}
                      />
                    ))}
                  </div>
                ...
                <div className="absolute inset-0 rounded-xl bg-neon-purple/5 opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
            ))}
          </div>
        )}
...
export default MinzzleSwipesHexLevelsPage;
